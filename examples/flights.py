...
GOALS = (
    "Find one-way flights from Zurich to London on September 27, 2026, for one adult in economy. "
...
def verify(page):
    ...
    try:
        date_in_url = b"2026-09-27" in base64.urlsafe_b64decode(encoded + "=" * (-len(encoded) % 4))
    except ValueError:
        date_in_url = False
    ...
    checks = {
        ...
        "date": values.get("Departure") == "Sun, Sep 27",
        "year": date_in_url or "departing 2026-09-27" in page["text"],
        "results": bool(flights) and all("Sunday, September 27" in f for f in flights),
    }
