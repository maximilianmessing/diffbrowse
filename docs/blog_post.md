...
DiffBrowse cleanly separates browser observation from model execution through [`decision_request.py`](file:///Users/maxmessing/Development/opensource/jev-ultrafast/jev_ultrafast/decision_request.py) and [`structured_canvas.py`](file:///Users/maxmessing/Development/opensource/jev-ultrafast/jev_ultrafast/structured_canvas.py):
...
2. **AMD Strix Halo (ROCm 6.2+)**:
   ...
   - Eliminates PCIe bus transfer overhead completely. Turnkey setup via [`scripts/setup_strix_halo.sh`](file:///Users/maxmessing/Development/opensource/jev-ultrafast/scripts/setup_strix_halo.sh) and [`docker/Dockerfile.rocm`](file:///Users/maxmessing/Development/opensource/jev-ultrafast/docker/Dockerfile.rocm).
3. **NVIDIA GPUs (CUDA 12.4+)**:
   - Compatible with single 24 GB cards (RTX 3090, RTX 4090) and datacenter accelerators (A100, H100).
   - Turnkey setup via [`scripts/setup_nvidia_cuda.sh`](file:///Users/maxmessing/Development/opensource/jev-ultrafast/scripts/setup_nvidia_cuda.sh) and [`docker/Dockerfile.cuda`](file:///Users/maxmessing/Development/opensource/jev-ultrafast/docker/Dockerfile.cuda).
